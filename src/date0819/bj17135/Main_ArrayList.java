...
public class Main_ArrayList {
	...
	public static void placeArcher(int depth, int cnt) {
		if (depth == 3) { // 세명의 궁수를 골랐으면
			...
			for (int i = 0; i < N; i++) { // 적이 전진 (N번 공격할 수 있음)
				...
				ArrayList<int[]> pos = new ArrayList<>(); // 죽일 적을 모아놨다가 한번에 죽임 (같은 적이 여러 궁수에게 공격 당할 수 있다는 조건)
				for (int idx = 0; idx < 3; idx++) {
					int archerX = N;
					int archerY = archers[idx]; // 현재 궁수의 위치
					int[] enemy_pos = find(archerX, archerY);
					pos.add(enemy_pos); // 공격한 적의 위치
				}

				for (int k = 0; k < pos.size(); k++) {
					int[] curTarget = pos.get(k);
					if (curTarget[0] == -1 && curTarget[1] == -1)
						continue; // (-1, -1)은 적을 발견하지 못했을 경우
	...
	public static int[] find(int x, int y) {
		...
		int[] ans = new int[] { targetX, targetY };
		return ans;
	}
